...
class Progress:
    def __init__(self, progress_file: str | None = None) -> None:
    ...
    def emit(self, value: int) -> None:
        ...
        now = time.monotonic()
        if self.completed < self.total and now - self.last_write < 0.1:
            return

        temporary_path = f"{self.progress_file}.tmp"
        with open(temporary_path, "w", encoding="utf-8") as progress:
            progress.write(f"{self.completed},{self.total}\n")
...
"""
    ...
    video_settings = VideoSettings(VideoExportMode.SMARTCUT, QUALITY_PRESETS[args.quality], 'copy')
